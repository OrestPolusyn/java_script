 ...
 */
const getUsersByGender = (arr, gender) => users.filter(user => user.gender === gender).map(elem => elem.name);
 ...
 */
const getUserByEmail = (arr, email) => users.filter(user => user.email === email).map(elem => elem.name);
...
//  */
const getTotalBalance = arr => users.reduce((acc, elem) => (acc += elem.balance), 0);
...
//  */
const getUsersByFriend = (arr, name) => users.filter(user => user.friends.includes(name)).map(user => user.name);
